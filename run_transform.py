...
######## FIXTURES
fixtures_cols = {'code': 'fixture_id_long',
                 'event': 'gameweek',
...
df_fixtures.rename(columns=fixtures_cols, inplace=True)
...
df_gameweeks.drop(columns=['chip_plays', 'top_element_info'] + gameweek_drop,
                  inplace=True)
df_gameweeks.rename(columns=gameweek_cols, inplace=True)


df_gameweeks[gameweek_str_cols] = df_gameweeks[gameweek_str_cols]\
    .applymap(lambda x: re.sub(r'(\.\d+)', '', str(x)) if not np.isnan(x) else np.nan)
...
df_players_sum[players_sum_str_cols] = df_players_sum[players_sum_str_cols].astype(str)
df_players_sum.sort_values(players_sum_index, inplace=True)


df_players_append = df_players_sum.copy()



player_id_mapping = df_players_sum.copy()[['player_id', 'player_id_long']]
...
df_players_prev_seasons.rename(columns=players_prev_seasons_rename, inplace=True)
df_players_prev_seasons.drop(columns=players_prev_seasons_drop, inplace=True)
df_players_prev_seasons[players_prev_seasons_str_cols] = df_players_prev_seasons[players_prev_seasons_str_cols].astype(str)
...
players_future_str_cols = ['fixture_id_long', 'gameweek', 'home_team_id', 'away_team_id']
...
df_players_future['kickoff_datetime'] = pd.to_datetime(df_players_future['kickoff_time'],
                                                     errors='coerce')
...
df_players_past2 = pd.merge(df_players_past, df_fixtures[
    ['fixture_id', 'fixture_id_long', 'away_team_id', 'home_team_id']],
                            how='inner',
                            right_on='fixture_id',
                            left_on='fixture_id'
                            )
df_players_future2 = pd.merge(df_players_future, df_fixtures[['fixture_id', 'fixture_id_long']],
                            how='inner',
                            on='fixture_id_long'
                            )
...
df_players_full = pd.concat((df_players_past2, df_players_future2), sort=False)
